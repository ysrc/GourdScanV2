...
pip install redis    
pip install scapy    
sqlmapapi   git clone sqlmap源码     
根据自己的系统安装redis apt-get install redis-server 或者自行编译 https://redis.io/download
...
###docker
    ...
    build之后，执行命令docker run -d -p 10000:22 c42a228eea05 /usr/sbin/sshd -D
    然后ssh root@localhost -p 10000即可登录进入容器，用户名root，密码Y3rc_admin
...
###关于系统其他选项：
    1.只允许同时有200个session(session_size=6600)存在，为了方便，session过期时间为300天。不过以上
    设置均可以在config里面自行设置。
